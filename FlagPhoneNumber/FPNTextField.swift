...
open class FPNTextField: UITextField {
    ...
    open var borderLineView = UIView(frame: CGRect(x: 34, y: 8, width: 1, height: 24))
    ...
    open func addleftLine(isRightToLeft: Bool, backgroundColor: UIColor) {
        if isRightToLeft {
            borderLineView = UIView(frame: CGRect(x: 10, y: -3, width: 1, height: 24))
        }
        borderLineView.backgroundColor = backgroundColor
        self.leftView?.addSubview(borderLineView)
        flagButtonSize.width = flagButtonSize.width + 10.0
        flagWidthConstraint?.constant = flagWidthConstraint?.constant ?? 0.0 + 10.0
        flagWidthConstraint = NSLayoutConstraint(item: flagButton, attribute: .width, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 0, constant: flagButtonSize.width)
        flagWidthConstraint?.isActive = true

        flagButton.imageEdgeInsets = isRightToLeft ? UIEdgeInsets(top: 0, left: 0, bottom: 0, right: -17) : UIEdgeInsets(top: 0, left: -17, bottom: 0, right: 0)
    }
    ...
    func checkPhoneValidity() -> Bool {
        if let phoneCode = selectedCountry?.phoneCode, let number = text {
            ...
            if let validPhoneNumber = getValidNumber(phoneNumber: cleanedPhoneNumber) {
                ...
                cleanedPhoneNumber = "\(selectedCountry?.phoneCodeForValidating ?? "")\(validPhoneNumber.nationalNumber.stringValue)"
                if getValidNumber(phoneNumber: cleanedPhoneNumber) != nil {
                    return true
                }
                return false
            } else {
